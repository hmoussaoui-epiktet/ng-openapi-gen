...
export class Operation {
	...
	constructor(
		...
		const allParams = [...this.collectParameters(false, pathSpec.parameters), ...this.collectParameters(true, spec.parameters)];
	...
	private getResponse(responseObject: ResponseObject | ReferenceObject, statusCode: string): Response {
		...
		const response = new Response(statusCode, responseDesc.description || '', this.collectContent(responseDesc.content), this.options);
			...
			if (content && content.length > 0) {
				for (const type of content) {
					if (type && type.mediaType) {
						...
						if (map.has(part)) {
							this.logger.warn(
								`Overwriting variant method part '${part}' for media type '${map.get(part)?.mediaType}' by media type '${type.mediaType
								}'.`,
							);
						}
	...
	private calculateVariants() {
		...
		const hasAmbiguity =
			totalVariants > 1 &&
			requestVariants.size === 1 &&
			responseVariants.size === 1 &&
			[...requestVariants.keys()][0] === '' &&
			[...responseVariants.keys()][0] === '' &&
			[...requestVariants.values()][0] !== null &&
			[...responseVariants.values()][0] !== null;
