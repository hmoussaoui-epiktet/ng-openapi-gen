...
export interface ValidatorMappingEntry {
	...
	template: string;
	/** Optional import path for this validator. If not specified, uses defaultImportPath. */
	importPath?: string;
}
...
export interface ValidationConfig {
	...
	/**
	 * Generation mode:
	 * - 'function': generates `export function applyXxxValidation<T>(p: SchemaPath<T>): void { ... }`
	 * - 'schema': generates `export const xxxSchema = schema<Xxx>((rootPath) => { ... })`
	 * Defaults to 'function'.
	 */
	mode?: 'function' | 'schema';
	/** Output directory for validation files. Defaults to 'validation'. */
	outputDir?: string;
	/** Default import path for validators without specific importPath. Defaults to '@angular/forms/signals'. */
	defaultImportPath?: string;
	/** Dictionary mapping OpenAPI constraints to Signal Form validators. Each entry can have its own importPath. */
	...
	/** Suffix for generated validation functions (mode 'function' only). Defaults to 'Validation'. */
	functionSuffix?: string;
	/** Suffix for generated schema constants (mode 'schema' only). Defaults to 'ProfileSchema'. */
	schemaSuffix?: string;
	/** When true, uses x-validators extension from OpenAPI schema (generated by C# ValidationSchemaFilter) instead of standard OpenAPI constraints. Default is false. */
	addSchemaSignalForm?: boolean;
	/** The key name for the validators extension in the OpenAPI schema. Defaults to 'x-validators'. */
	xValidatorsKey?: string;
	/** When true, generates a Schemas helper object (like Default for factories) that exports all validation schemas. Default is false. */
	generateSchemasHelper?: boolean;
}
