...
describe('Generation tests using default-factories.json', () => {
	...
	it('WithDefaults model should use OpenAPI default values', () => {
		const withDefaults = gen.models.get('WithDefaults');
		expect(withDefaults).toBeDefined();
		if (withDefaults) {
			const ts = gen.templates.apply('model', withDefaults);
			expect(ts).toContain('export function withDefaultsDefault(): WithDefaults {');
			// Should use OpenAPI default values
			expect(ts).toContain('name: \'John Doe\'');
			expect(ts).toContain('age: 25');
			expect(ts).toContain('score: 99.5');
			expect(ts).toContain('isVerified: true');
			expect(ts).toContain('tags: ["tag1","tag2"]');
			// Property without default should use computed default
			expect(ts).toContain('noDefault: \'\'');
		}
	});

	it('CandidatDTO model with oneOf nullable enum should use null as default', () => {
		const candidatDTO = gen.models.get('CandidatDTO');
		expect(candidatDTO).toBeDefined();
		if (candidatDTO) {
			const ts = gen.templates.apply('model', candidatDTO);
			expect(ts).toContain('export function candidatDTODefault(): CandidatDTO {');
			// oneOf with null and $ref to enum should default to null
			expect(ts).toContain('codeJustificatifDiplome: null');
			// Required enum without nullable should use first enum value
			expect(ts).toContain('requiredEnumField: 1');
			// oneOf with null and string should default to null
			expect(ts).toContain('nullableString: null');
		}
	});

	it('CodeJustificatifDiplome enum should not have default factory', () => {
		const codeEnum = gen.models.get('CodeJustificatifDiplome');
		expect(codeEnum).toBeDefined();
		if (codeEnum) {
			const ts = gen.templates.apply('model', codeEnum);
			// Enums should not have default factories
			expect(ts).not.toContain('Default()');
		}
	});
