import { ClassDeclaration, InterfaceDeclaration, TypeAliasDeclaration, TypescriptParser } from 'typescript-parser';
import { NgOpenApiGen } from '../lib/ng-openapi-gen';
import { OpenAPIObject } from '../lib/openapi-typings';
import options from './petstore-3.1.config.json';
import petstoreSpec from './petstore-3.1.json';

const spec = petstoreSpec as OpenAPIObject;
const gen = new NgOpenApiGen(spec, options);
gen.generate();


describe('Generation tests using petstore-3.1.json', () => {
	...
	it('pets tag', () => {
		...
		parser.parseSource(ts).then(ast => {
			...
			if (listPets) {
				...
				expect(type).toEqual('ListPetsParams');
			}
			...
			if (createPets) {
				...
				expect(type).toEqual('CreatePetsParams');
			}
			...
			if (showPetById) {
				...
				expect(type).toEqual('ShowPetByIdParams');
			}
